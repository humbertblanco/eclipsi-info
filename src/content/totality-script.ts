...
function twilightComparison(lux: number, atmosphere: Atmosphere): LocalisedText {
  ...
  if (alt >= 10) return { ca: 'un dia ennuvolat', es: 'un día nublado', en: 'an overcast day', fr: 'une journée nuageuse' };
  if (alt >= 0) return { ca: 'els minuts abans de la posta de sol', es: 'los minutos antes de la puesta de sol', en: 'the minutes before sunset', fr: 'les minutes avant le coucher du soleil' };
  if (alt >= -6) return { ca: 'el crepuscle just després de la posta', es: 'el crepúsculo justo después de la puesta', en: 'twilight just after sunset', fr: 'crépuscule juste après le coucher du soleil' };
  if (alt >= -12) {
    return {
      ...
      en: 'the end of civil twilight, when the street lights come on', fr: 'la fin du crépuscule civil, quand les lampadaires s\'allument',
    };
  }
  if (alt >= -18) return { ca: 'el crepuscle nàutic', es: 'el crepúsculo náutico', en: 'nautical twilight', fr: 'crépuscule nautique' };
  return { ca: 'la nit tancada', es: 'la noche cerrada', en: 'full darkness', fr: 'obscurité totale' };
}
...
function bodyName(name: string, locale: TimerLocale): string {
  ...
  if (locale === 'fr') return BODY_FR[name] ?? name;
  return name;
}
...
function bodyPlacement(body: VisibleBody, sunAz: number, sunAlt: number, locale: TimerLocale): string {
  ...
  const side = dAz >= 0
    ? locale === 'es' ? 'a la derecha' : locale === 'en' ? 'to the right' : locale === 'fr' ? 'à droite' : 'a la dreta'
    : locale === 'es' ? 'a la izquierda' : locale === 'en' ? 'to the left' : locale === 'fr' ? 'à gauche' : 'a l’esquerra';
  const horizontal = `${Math.round(Math.abs(dAz))}° ${side}`;

  let vertical: string;
  if (Math.abs(dAlt) < 3) {
    vertical = locale === 'es' ? 'a la misma altura' : locale === 'en' ? 'at the same height' : locale === 'fr' ? 'à la même hauteur' : 'a la mateixa altura';
  } else if (dAlt > 0) {
    vertical = locale === 'es' ? `${Math.round(dAlt)}° más arriba` : locale === 'en' ? `${Math.round(dAlt)}° higher` : locale === 'fr' ? `${Math.round(dAlt)}° plus haut` : `${Math.round(dAlt)}° més amunt`;
  } else {
    vertical = locale === 'es' ? `${Math.round(-dAlt)}° más abajo` : locale === 'en' ? `${Math.round(-dAlt)}° lower` : locale === 'fr' ? `${Math.round(-dAlt)}° plus bas` : `${Math.round(-dAlt)}° més avall`;
  }

  const conjunction = locale === 'es' ? 'y' : locale === 'en' ? 'and' : locale === 'fr' ? 'et' : 'i';
  const sunReference = locale === 'en' ? 'of the Sun' : locale === 'fr' ? 'du Soleil' : 'del Sol';
...
function totalityBeats(
  ...
  drafts.push({
    ...
    look: 'ground',
    title: { ca: 'La llum es torna metàl·lica', es: 'La luz se vuelve metálica', en: 'The light turns metallic', fr: 'La lumière devient métallique' },
    text: {
      ...
      en: 'In the final minutes the Sun is no longer a disc but a line of light, and shadows become extraordinarily sharp. Look at the ground beneath a tree: every gap between the leaves acts as a pinhole and fills the ground with crescents.', fr: 'Dans les dernières minutes, le Soleil n\'est plus un disque mais une ligne de lumière et les ombres deviennent extraordinairement nettes. Regardez le sol sous un arbre : chaque espace entre les feuilles fait office de trou d\'épingle et remplit le sol de croissants.',
    },
    speech: {
      ca: 'Mira les ombres a terra. Es tornen molt nítides.',
      es: 'Mira las sombras en el suelo. Se vuelven muy nítidas.',
      en: 'Look at the shadows on the ground. They are becoming very sharp.', fr: 'Regardez les ombres au sol. Ils deviennent très pointus.',
    },
  });
  ...
  drafts.push({
    ...
    filterState: 'away-from-sun',
    look: 'ground',
    title: { ca: 'Bandes d’ombra', es: 'Bandas de sombra', en: 'Shadow bands', fr: 'Bandes d\'ombre' },
    text: {
      ...
      en: 'Rippling dark lines race across the ground like patterns on the bottom of a swimming pool. They are caused by atmospheric turbulence and do not always appear. A pale, smooth surface helps reveal them: this is what the white sheet spread on the ground is for.', fr: 'Des lignes sombres ondulantes courent sur le sol comme des motifs au fond d’une piscine. Ils sont provoqués par les turbulences atmosphériques et n’apparaissent pas toujours. Une surface pâle et lisse contribue à les révéler : c\'est à cela que sert le drap blanc étalé au sol.',
    },
    speech: { ca: 'Mira el llençol blanc. Bandes d’ombra.', es: 'Mira la sábana blanca. Bandas de sombra.', en: 'Look at the white sheet. Shadow bands.', fr: 'Regardez le drap blanc. Bandes d\'ombre.' },
  });
  ...
  drafts.push({
    ...
    look: 'sun',
    title: { ca: 'Grans de Baily', es: 'Granos de Baily', en: 'Baily’s beads', fr: 'Les perles de Baily' },
    text: {
      ...
      en: 'The last thread of sunlight breaks into separate bright points: sunlight is passing between mountains along the Moon’s edge. This is still the photosphere, so keep the filter on.', fr: 'Le dernier fil de lumière solaire se divise en points lumineux séparés : la lumière solaire passe entre les montagnes le long du bord de la Lune. Il s\'agit toujours de la photosphère, alors gardez le filtre activé.',
    },
    speech: { ca: 'Grans de Baily. Amb el filtre posat.', es: 'Granos de Baily. Con el filtro puesto.', en: 'Baily’s beads. Keep the filter on.', fr: 'Les perles de Baily. Gardez le filtre en place.' },
  });
  ...
  drafts.push({
    ...
    look: 'sun',
    essential: true,
    title: { ca: 'Anell de diamant', es: 'Anillo de diamante', en: 'Diamond ring', fr: 'Bague diamant' },
    text: {
      ...
      en: 'One point of light remains, with the corona beginning to show around it. This is the image everyone knows. It is still the photosphere: remove the filter when the voice tells you, not when it merely seems less dazzling.', fr: 'Il reste un point de lumière, avec la couronne qui commence à apparaître autour de lui. C\'est l\'image que tout le monde connaît. C\'est toujours la photosphère : enlevez le filtre quand la voix vous le dit, pas quand elle paraît simplement moins éblouissante.',
    },
    speech: { ca: 'Anell de diamant. Filtre posat.', es: 'Anillo de diamante. Filtro puesto.', en: 'Diamond ring. Keep the filter on.', fr: 'Bague en diamant. Gardez le filtre en place.' },
  });
  ...
  drafts.push({
    ...
    text: {
      ...
      en: 'The solar disc is now completely covered. From now, and only until third contact, you may look without a filter: that is the only way to see the corona, which is completely invisible through a solar filter. Check before doing anything: if any point of light remains, keep the filter on. A pink arc —the chromosphere— may linger where the Sun disappeared, but it lasts about five seconds and the safety delay usually misses it.', fr: 'Le disque solaire est désormais entièrement recouvert. A partir de maintenant, et seulement jusqu\'au troisième contact, vous pouvez regarder sans filtre : c\'est le seul moyen de voir la couronne, qui est totalement invisible à travers un filtre solaire. Vérifiez avant de faire quoi que ce soit : s\'il reste un point lumineux, laissez le filtre allumé. Un arc rose – la chromosphère – peut persister là où le Soleil a disparu, mais il dure environ cinq secondes et le délai de sécurité ne le permet généralement pas.',
    },
  ...
  drafts.push({
    ...
    look: 'sun',
    essential: true,
    title: { ca: 'Corona', es: 'Corona', en: 'Corona', fr: 'Couronne' },
    text: {
      ...
      en: 'The reason for the journey. A pearly, structured halo with streamers extending several solar radii. No photograph looks like it: no sensor can capture the dynamic range your eye sees. Look with the naked eye first, then with binoculars.', fr: 'La raison du voyage. Un halo nacré et structuré avec des banderoles s\'étendant sur plusieurs rayons solaires. Aucune photographie ne lui ressemble : aucun capteur ne peut capturer la plage dynamique que voit votre œil. Regardez d’abord à l’œil nu, puis avec des jumelles.',
    },
    speech: { ca: 'Corona. Mira-la a ull nu.', es: 'Corona. Mírala a simple vista.', en: 'Corona. Look with the naked eye.', fr: 'Couronne. Regardez à l\'œil nu.' },
  });
  ...
  drafts.push({
    ...
    filterState: 'naked-eye',
    look: 'sun',
    title: { ca: 'Protuberàncies', es: 'Protuberancias', en: 'Prominences', fr: 'Proéminences' },
    text: {
      ...
      en: 'Reddish tongues of plasma protruding from the Moon’s edge. They show clearly through binoculars —now without a filter, but only while totality lasts.', fr: 'Des langues rougeâtres de plasma dépassant du bord de la Lune. Ils apparaissent clairement à travers des jumelles – désormais sans filtre, mais seulement tant que dure la totalité.',
    },
    speech: { ca: 'Protuberàncies a la vora. Prismàtics, si en tens.', es: 'Protuberancias en el borde. Prismáticos, si tienes.', en: 'Prominences at the edge. Use binoculars if you have them.', fr: 'Protubérances en bordure. Utilisez des jumelles si vous en avez.' },
  });
  ...
  if (durationSec >= FULL_SEQUENCE_MIN_SEC) {
    ...
    drafts.push({
      ...
      filterState: 'away-from-sun',
      look: 'around',
      title: { ca: 'Crepuscle de 360°', es: 'Crepúsculo de 360°', en: '360° twilight', fr: 'Crépuscule à 360°' },
      ...
      text: {
        ...
        en: 'Look away from the Sun for a moment and turn around. The entire horizon has the colours of sunset in every direction at once: you are beneath a shadow only a few hundred kilometres wide, and beyond it daylight continues.', fr: 'Détournez votre regard du Soleil pendant un moment et retournez-vous. L\'horizon tout entier a les couleurs du coucher du soleil dans toutes les directions à la fois : vous êtes sous une ombre de seulement quelques centaines de kilomètres de large, et au-delà, la lumière du jour continue.',
      },
      speech: { ca: 'Gira’t. Tot l’horitzó és color de posta.', es: 'Date la vuelta. Todo el horizonte es color de puesta.', en: 'Turn around. The whole horizon has the colours of sunset.', fr: 'Faire demi-tour. Tout l\'horizon a les couleurs du coucher du soleil.' },
    });
  }
  ...
  drafts.push({
    ...
    filterState: 'away-from-sun',
    look: 'around',
    title: { ca: 'Fred, vent i silenci', es: 'Frío, viento y silencio', en: 'Cold, wind and silence', fr: 'Froid, vent et silence' },
    text: {
      ...
      en: `The temperature drops sharply within seconds and a gust of wind often rises. Birds suddenly fall silent or settle down to sleep, while crickets start singing. The light will have fallen to about ${formatLux(totalityLux)} lux, like ${totalityLight.en}: that is what our model gives with the Sun at ${Math.round(sunAlt)}°, and the lower the Sun, the less light enters the shadow. Listen as much as you look.`, fr: `La température chute brusquement en quelques secondes et une rafale de vent se lève souvent. Les oiseaux se taisent ou s’endorment soudainement, tandis que les grillons se mettent à chanter. La lumière sera tombée à environ ${formatLux(totalityLux)} lux, comme ${totalityLight.fr} : c’est ce que donne notre modèle avec le Soleil à ${Math.round(sunAlt)}°, et plus le Soleil est bas, moins de lumière entre dans l’ombre. Écoutez autant que vous regardez.`,
    },
    speech: { ca: 'Escolta. Els ocells callen i baixa la temperatura.', es: 'Escucha. Los pájaros callan y baja la temperatura.', en: 'Listen. The birds fall silent and the temperature drops.', fr: 'Écouter. Les oiseaux se taisent et la température baisse.' },
  });
  ...
  if (durationSec >= FULL_SEQUENCE_MIN_SEC) {
    drafts.push({
      ...
      filterState: 'naked-eye',
      look: 'sun',
      title: { ca: 'Deixa la càmera', es: 'Deja la cámara', en: 'Put the camera down', fr: 'Posez la caméra' },
      text: {
        ...
        en: 'If you have been using the camera, stop now. These final seconds are for looking. Millions of better photographs of the corona already exist; you will never see this corona, with these people and this horizon, again.', fr: 'Si vous utilisez l\'appareil photo, arrêtez maintenant. Ces dernières secondes sont à chercher. Il existe déjà des millions de meilleures photographies de la couronne ; vous ne reverrez plus jamais ce corona, avec ces gens et cet horizon.',
      },
      speech: { ca: 'Deixa la càmera. Mira la corona.', es: 'Deja la cámara. Mira la corona.', en: 'Put the camera down. Look at the corona.', fr: 'Posez la caméra. Regardez la couronne.' },
    });
  }
  ...
  drafts.push({
    ...
    filterState: 'naked-eye',
    look: 'sun',
    title: { ca: 'L’arc rosa de sortida', es: 'El arco rosa de salida', en: 'The returning pink arc', fr: 'L\'arc rose qui revient' },
    ...
    text: {
      ...
      en: 'Watch the opposite edge, where the Sun will return: when a pink arc appears there, totality is ending and the photosphere will emerge next. Do not wait to see it before replacing the filter. What you saw governed removing the filter; the clock governs putting it back on, and the clock acts early.', fr: 'Observez le bord opposé, là où le Soleil reviendra : lorsqu\'un arc rose y apparaît, la totalité se termine et la photosphère émergera ensuite. N\'attendez pas de le voir avant de remplacer le filtre. Ce que vous avez vu régissait le retrait du filtre ; l\'horloge commande sa remise en marche, et l\'horloge agit tôt.',
    },
    speech: { ca: 'Vigila la vora. Quan torni l’arc rosa, s’acaba.', es: 'Vigila el borde. Cuando vuelva el arco rosa, se acaba.', en: 'Watch the edge. When the pink arc returns, totality is ending.', fr: 'Surveillez le bord. Lorsque l’arc rose revient, la totalité prend fin.' },
  });
      ...
      text:
        sec === 15
          ? {
              ...
              en: 'The Sun does not return gradually: it reappears suddenly, and the first point of photosphere is already too bright for a dark-adapted eye. The filter must be on BEFORE it appears, not when you see it.', fr: 'Le Soleil ne revient pas progressivement : il réapparaît soudainement, et le premier point de la photosphère est déjà trop brillant pour un œil adapté à l\'obscurité. Le filtre doit être activé AVANT qu’il apparaisse, et non pas lorsque vous le voyez.',
            }
          : {
              ...
              en: 'Cover your eyes now. Third contact is five seconds away, and contact times carry a couple of seconds of uncertainty.', fr: 'Couvrez-vous les yeux maintenant. Le troisième contact est dans cinq secondes, et les temps de contact comportent quelques secondes d\'incertitude.',
            },
  ...
  drafts.push({
    ...
    look: 'sun',
    essential: true,
    title: { ca: 'Anell de diamant de sortida', es: 'Anillo de diamante de salida', en: 'Outgoing diamond ring', fr: 'Bague diamant sortante' },
    text: {
      ...
      en: 'The first point of sunlight returns at the opposite edge, with the corona still faintly visible around it. View it through the filter: this is the photosphere again.', fr: 'Le premier point de lumière solaire revient sur le bord opposé, avec la couronne encore faiblement visible autour de lui. Regardez-le à travers le filtre : c\'est à nouveau la photosphère.',
    },
    speech: { ca: 'Anell de diamant. Només amb filtre.', es: 'Anillo de diamante. Solo con filtro.', en: 'Diamond ring. Through the filter only.', fr: 'Bague en diamant. Par le filtre uniquement.' },
  });
  ...
  drafts.push({
    ...
    filterState: 'filtered',
    look: 'sun',
    title: { ca: 'Grans de Baily de sortida', es: 'Granos de Baily de salida', en: 'Outgoing Baily’s beads', fr: 'Grains de Baily à la sortie' },
    text: {
      ...
      en: 'The points multiply along the edge until they become a continuous thread. Keep the filter on from now until fourth contact.', fr: 'Les points se multiplient le long du bord jusqu\'à devenir un fil continu. Gardez le filtre allumé jusqu\'au quatrième contact.',
    },
    speech: { ca: 'Grans de Baily. El filtre es queda posat.', es: 'Granos de Baily. El filtro se queda puesto.', en: 'Baily’s beads. Keep the filter on.', fr: 'Les perles de Baily. Gardez le filtre en place.' },
  });
  ...
  if (shadow) {
    ...
    const cardinalFr = bearingToCardinal(shadow.departureBearing, 'fr');
    drafts.push({
      id: 'shadow-departs',
      anchor: 'c3',
      offsetSec: 15,
      ...
      filterState: 'away-from-sun',
      look: 'horizon',
      title: { ca: 'L’ombra marxa', es: 'La sombra se va', en: 'The shadow departs', fr: 'L\'ombre s\'en va' },
      text: {
        ...
        en: `Turn towards the ${cardinalEn}. The shadow recedes that way as fast as it arrived, and you can watch the wall of darkness depart against a brightening sky.`, fr: `Tournez-vous vers ${cardinalFr}. L’ombre s’éloigne aussi vite qu’elle est arrivée, et vous pouvez regarder le mur d’obscurité partir sur un ciel qui s’éclaircit.`,
      },
      speech: {
        ca: `Mira cap ${towardsCa(cardinalCa)}. L’ombra marxa.`,
        es: `Mira hacia el ${cardinalEs}. La sombra se va.`,
        en: `Look towards the ${cardinalEn}. The shadow is departing.`, fr: `Regardez vers ${cardinalFr}. L’ombre s’éloigne.`,
      },
    });
  }
  ...
  drafts.push({
    ...
    filterState: 'away-from-sun',
    look: 'ground',
    title: { ca: 'Bandes d’ombra, segona passada', es: 'Bandas de sombra, segunda pasada', en: 'Shadow bands, second pass', fr: 'Bandes d\'ombre, deuxième passage' },
    text: {
      ...
      en: 'Shadow bands cross the ground again one minute after totality. This is your second and final chance to see them.', fr: 'Des bandes d\'ombre traversent à nouveau le sol une minute après la totalité. C\'est votre deuxième et dernière chance de les voir.',
    },
    speech: { ca: 'Torna a mirar el llençol. Bandes d’ombra.', es: 'Vuelve a mirar la sábana. Bandas de sombra.', en: 'Look at the sheet again. Shadow bands.', fr: 'Regardez à nouveau le drap blanc. Bandes d\'ombre.' },
  });
...
function shadowWallBeat(shadow: ShadowMotion | null): BeatDraft {
  const base = {
    ...
    essential: true,
    title: { ca: 'L’ombra que arriba', es: 'La sombra que llega', en: 'The approaching shadow', fr: 'L\'ombre qui approche' },
  };
  ...
  if (!shadow) {
    return {
      ...
      text: {
        ...
        en: 'Look away from the Sun and towards the horizon where the shadow is coming from. It is a wall of darkness approaching at supersonic speed. We cannot calculate its bearing from this location.', fr: 'Regardez loin du Soleil et vers l’horizon d’où vient l’ombre. C\'est un mur de ténèbres qui s\'approche à une vitesse supersonique. Nous ne pouvons pas calculer son azimut depuis cet emplacement.',
      },
      speech: { ca: 'Mira l’horitzó. L’ombra arriba.', es: 'Mira el horizonte. La sombra llega.', en: 'Look at the horizon. The shadow is coming.', fr: 'Regardez l\'horizon. L\'ombre arrive.' },
    };
  }
  ...
  const cardinalFr = bearingToCardinal(shadow.arrivalBearing, 'fr');
  ...
  const speedFr = shadow.speedDiverging ? 'très rapidement' : `à environ ${formatSpeed(shadow.speedKmh)} km/h`;
  ...
  const caveatEn = shadow.lowSunCaveat
    ...
    : '';
  const caveatFr = shadow.lowSunCaveat
    ? ` Avec le Soleil à ${Math.round(shadow.sunAltitudeDeg)}°, le mur arrive sous un angle très rasant : il paraît plus diffus et se distingue difficilement du crépuscule.`
    : '';
  ...
  return {
    ...
    text: {
      ...
      en: `Look away from the Sun and towards the ${cardinalEn}. The Moon’s shadow is arriving from there ${speedEn}: a wall of darkness approaches and engulfs you.${caveatEn}`, fr: `Détournez le regard du Soleil et regardez vers ${cardinalFr}. L’ombre de la Lune arrive de là ${speedFr} : un mur d’obscurité approche et vous engloutit.${caveatFr}`,
    },
    speech: {
      ca: `Mira cap ${towardsCa(cardinalCa)}. L’ombra arriba.`,
      es: `Mira hacia el ${cardinalEs}. La sombra llega.`,
      en: `Look towards the ${cardinalEn}. The shadow is coming.`, fr: `Regardez vers ${cardinalFr}. L’ombre arrive.`,
    },
  };
}
...
function planetsBeat(offsetSec: number, bodies: VisibleBody[], sunAz: number, sunAlt: number): BeatDraft {
  const base = {
    ...
    filterState: 'away-from-sun' as const,
    look: 'sky' as const,
    title: { ca: 'Planetes', es: 'Planetas', en: 'Planets', fr: 'Planètes' },
  };
  ...
  if (bodies.length === 0) {
    return {
      ...
      text: {
        ...
        en: 'From this location, no planet is calculated to be bright enough and above the horizon during totality. The sky will resemble civil twilight: a bright star may appear, but it is not worth spending precious seconds looking for one.', fr: 'À partir de cet emplacement, aucune planète n’est censée être suffisamment brillante et au-dessus de l’horizon pendant la totalité. Le ciel ressemblera à un crépuscule civil : une étoile brillante peut apparaître, mais cela ne vaut pas la peine de passer de précieuses secondes à en chercher une.',
      },
      speech: { ca: 'Des d’aquí no surt cap planeta brillant.', es: 'Desde aquí no sale ningún planeta brillante.', en: 'No bright planet appears from here.', fr: 'Aucune planète brillante n’apparaît depuis cet emplacement.' },
    };
  }
  ...
  const sideFr = azimuthDelta(shown[0].azimuth, sunAz) >= 0 ? 'à droite' : 'à gauche';
  ...
  return {
    ...
    text: {
      ...
      en: `Bright planets appear in a sky as dark as civil twilight. Calculated for this location and time: ${listEn}.`, fr: `Des planètes brillantes apparaissent dans un ciel aussi sombre que le crépuscule civil. Positions calculées pour ce lieu et cette heure : ${listFr}.`,
    },
    speech: {
      ca: `${firstCa}, ${sideCa} del Sol.`,
      es: `${firstEs}, ${sideEs} del Sol.`,
      en: `${firstEn}, ${sideEn} of the Sun.`, fr: `${firstFr}, ${sideFr} du Soleil.`,
    },
  };
}
  ...
  return [
    {
      ...
      text: {
        ...
        en: 'In the final minutes the light turns metallic and shadows become very sharp. It is still daylight and will remain so throughout: that is the difference from a total eclipse.', fr: 'Dans les dernières minutes, la lumière devient métallique et les ombres deviennent très nettes. Il fait encore jour et le restera tout au long de la journée : c\'est la différence avec une éclipse totale.',
      },
      speech: { ca: 'Mira les ombres a terra. Es tornen molt nítides.', es: 'Mira las sombras en el suelo. Se vuelven muy nítidas.', en: 'Look at the shadows on the ground. They are becoming very sharp.', fr: 'Regardez les ombres au sol. Ils deviennent très pointus.' },
    },
    {
      id: 'ring-closes',
      anchor: 'c2',
      ...
      look: 'sun',
      essential: true,
      title: { ca: 'Es tanca l’anell', es: 'Se cierra el anillo', en: 'The ring closes', fr: 'L\'anneau se ferme' },
      text: {
        ...
        en: 'The Moon has just moved fully inside the solar disc and the thread of light closes into a complete ring. Keep the filter on: you are seeing the entire Sun around the edge.', fr: 'La Lune vient de se déplacer complètement à l’intérieur du disque solaire et le fil de lumière se referme en un anneau complet. Gardez le filtre activé : vous voyez tout le Soleil autour du bord.',
      },
      speech: { ca: 'Es tanca l’anell. Amb el filtre posat.', es: 'Se cierra el anillo. Con el filtro puesto.', en: 'The ring closes. Keep the filter on.', fr: 'L\'anneau se ferme. Gardez le filtre en place.' },
    },
    {
      id: 'never-safe',
      anchor: 'c2',
      ...
      essential: true,
      validForMs: 10 * SEC,
      title: { ca: 'El filtre no es toca', es: 'El filtro no se toca', en: 'Keep the filter on', fr: 'Gardez le filtre allumé' },
      text: {
        ...
        en: 'There is no safe moment without a filter during an annular eclipse. The ring is photosphere and burns just like the full Sun, even if the sky has darkened and it no longer seems dazzling. If you saw totality in 2026 or 2027, this is the day not to repeat that gesture.', fr: 'Il n\'y a pas de moment sûr sans filtre lors d\'une éclipse annulaire. L\'anneau est photosphère et brûle comme le plein Soleil, même si le ciel s\'est assombri et qu\'il ne semble plus éblouissant. Si vous avez vu la totalité en 2026 ou 2027, c’est le jour pour ne pas répéter ce geste.',
      },
      speech: {
        ca: 'Anularitat. El filtre no es treu en cap moment.',
        es: 'Anularidad. El filtro no se quita en ningún momento.',
        en: 'Annularity. Keep the filter on at all times.', fr: 'Annularité. Gardez le filtre allumé à tout moment.',
      },
    },
    {
      id: 'ring-thinnest',
      ...
      filterState: 'filtered',
      look: 'sun',
      title: { ca: 'L’anell, al màxim', es: 'El anillo, en el máximo', en: 'The ring at maximum', fr: 'La bague au maximum' },
      text: {
        ...
        en: 'At maximum, the ring is at its thinnest and most symmetrical. Through the filter —and filtered binoculars if you have them— its uneven thickness is visible: the Moon’s edge is not a smooth circle.', fr: 'Au maximum, l’anneau est le plus fin et le plus symétrique. À travers le filtre – et les jumelles filtrées si vous en avez – son épaisseur inégale est visible : le bord de la Lune n’est pas un cercle lisse.',
      },
      speech: { ca: 'L’anell és al més prim. Amb filtre.', es: 'El anillo está en su punto más fino. Con filtro.', en: 'The ring is at its thinnest. Keep the filter on.', fr: 'La bague est la plus fine. Gardez le filtre en place.' },
    },
    {
      id: 'crescents',
      anchor: 'c2',
      ...
      filterState: 'away-from-sun',
      look: 'ground',
      title: { ca: 'Anells a terra', es: 'Anillos en el suelo', en: 'Rings on the ground', fr: 'Anneaux au sol' },
      text: {
        ...
        en: 'Beneath a tree, or through a kitchen colander, every small hole projects the Sun’s shape: complete rings right now. This is the only way to see the ring’s shape without looking at it.', fr: 'Sous un arbre ou dans une passoire de cuisine, chaque petit trou projette la forme du Soleil : des anneaux complets en ce moment. C’est le seul moyen de voir la forme de la bague sans la regarder.',
      },
      speech: { ca: 'Mira a terra. Cada foradet projecta un anell.', es: 'Mira al suelo. Cada agujerito proyecta un anillo.', en: 'Look at the ground. Every small hole projects a ring.', fr: 'Regardez le sol. Chaque petit trou projette un anneau.' },
    },
    {
      id: 'temperature',
      anchor: 'c3',
      ...
      filterState: 'away-from-sun',
      look: 'around',
      title: { ca: 'Fred i silenci', es: 'Frío y silencio', en: 'Cold and silence', fr: 'Froid et silence' },
      text: {
        ...
        en: 'The temperature falls and a wind often rises. Birds grow quiet. The effect is gentler than at totality, but it is noticeable with the Sun low on a January afternoon.', fr: 'La température baisse et le vent se lève souvent. Les oiseaux se taisent. L\'effet est plus doux que dans la totalité, mais il est perceptible avec le soleil bas un après-midi de janvier.',
      },
      speech: { ca: 'Escolta. Baixa la temperatura.', es: 'Escucha. Baja la temperatura.', en: 'Listen. The temperature is falling.', fr: 'Écoutez. La température baisse.' },
    },
    {
      id: 'ring-opens',
      anchor: 'c3',
      ...
      look: 'sun',
      essential: true,
      title: { ca: 'S’obre l’anell', es: 'Se abre el anillo', en: 'The ring opens', fr: 'L\'anneau s\'ouvre' },
      text: {
        ...
        en: 'The Moon touches the opposite edge and breaks the ring. Annularity is over. Keep the filter on until fourth contact.', fr: 'La Lune touche le bord opposé et brise l\'anneau. L’annularité est terminée. Gardez le filtre allumé jusqu\'au quatrième contact.',
      },
      speech: { ca: 'S’obre l’anell. El filtre segueix posat.', es: 'Se abre el anillo. El filtro sigue puesto.', en: 'The ring opens. Keep the filter on.', fr: 'L\'anneau s\'ouvre. Gardez le filtre en place.' },
    },
  ];
}
  ...
  switch (gate.reason) {
    case 'partial-only':
      return {
        ...
        en: 'The eclipse is only partial from this location: the Moon never fully covers the Sun and some photosphere remains visible throughout. There is no safe moment without a filter, however high the obscuration.', fr: 'L\'éclipse n\'est que partielle à partir de cet endroit : la Lune ne recouvre jamais complètement le Soleil et une partie de la photosphère reste visible partout. Il n’y a pas de moment sûr sans filtre, aussi élevé que soit l’obscurcissement.',
      };
    case 'central-blocked-by-terrain':
      return {
        ...
        en: 'There is a central phase, but terrain blocks the Sun from this location while it lasts. Anything that reappears above the ridge may be photosphere: keep the filter on. Check whether moving a few kilometres would give you a view.', fr: 'Il y a une phase centrale, mais le terrain bloque le Soleil de cet endroit tant qu\'elle dure. Tout ce qui réapparaît au-dessus de la crête peut être de la photosphère : laissez le filtre allumé. Vérifiez si un déplacement de quelques kilomètres vous donnerait une vue.',
      };
    case 'totality-too-short':
      return {
        ...
        en: 'You are almost exactly on the edge of the path. The calculated totality is shorter than the uncertainty in the Moon’s mountain profile, so photosphere may remain visible for much of it. Removing the filter is not authorised here. Move towards the centre of the path.', fr: 'Vous êtes presque exactement au bord du chemin. La totalité calculée est plus courte que l’incertitude du profil des montagnes de la Lune, donc la photosphère peut rester visible pour une grande partie. Le retrait du filtre n\'est pas autorisé ici. Avancez vers le centre du chemin.',
      };
    case 'edge-uncertain':
      return {
        ...
        en: 'You are within the two- or three-kilometre strip where our error in the Sun–Moon relative position exceeds your margin from the edge. Put plainly: we do not know whether totality will occur here, and a fifty-fifty answer cannot authorise anyone to remove a filter. Move towards the centre of the path and the script will update automatically.', fr: 'Vous vous trouvez dans la bande de deux ou trois kilomètres où notre erreur dans la position relative Soleil-Lune dépasse votre marge par rapport au bord. En clair : nous ne savons pas si la totalité se produira ici, et une réponse 50/50 ne peut autoriser personne à supprimer un filtre. Déplacez-vous vers le centre du chemin et le script se mettra à jour automatiquement.',
      };
    case 'missing-central-contacts':
      return {
        ...
        en: 'We could not resolve second and third contact from this location. Without knowing exactly when totality begins and ends, removing the filter cannot be authorised.', fr: 'Nous n\'avons pas pu résoudre le deuxième et le troisième contact à partir de cet endroit. Sans savoir exactement quand commence et se termine la totalité, la suppression du filtre ne peut être autorisée.',
      };
    default:
      return {
        ca: 'Des d’aquest punt el filtre no es treu en cap moment.',
        es: 'Desde este punto el filtro no se quita en ningún momento.',
        en: 'Keep the filter on at all times from this location.', fr: 'Gardez le filtre allumé à tout moment à partir de cet endroit.',
      };
  }
}
...
function filteredBeats(
  ...
  const drafts: BeatDraft[] = [
    {
      ...
      essential: true,
      validForMs: 30 * SEC,
      title: { ca: 'Aquí el filtre no es treu', es: 'Aquí el filtro no se quita', en: 'Keep the filter on here', fr: 'Gardez le filtre ici' },
    ...
    },
    {
      ...
      filterState: 'away-from-sun',
      look: 'ground',
      title: { ca: 'Mitges llunes a terra', es: 'Medias lunas en el suelo', en: 'Crescents on the ground', fr: 'Croissants au sol' },
      text: {
        ...
        en: 'Beneath a tree, every gap between the leaves acts as a pinhole and fills the ground with tiny bitten Suns. A kitchen colander makes dozens at once. It is safe and the best way to show the eclipse to anyone without glasses.', fr: 'Sous un arbre, chaque espace entre les feuilles agit comme un trou d’épingle et remplit le sol de minuscules soleils mordus. Une passoire de cuisine en fait des dizaines à la fois. C\'est sûr et c\'est le meilleur moyen de montrer l\'éclipse à toute personne sans lunettes.',
      },
      speech: { ca: 'Mira a terra sota un arbre. Solets mossegats.', es: 'Mira al suelo bajo un árbol. Solecitos mordidos.', en: 'Look at the ground beneath a tree. Tiny crescent Suns.', fr: 'Regardez le sol sous un arbre. Petits Soleils en croissant.' },
    },
    {
      ...
      filterState: 'away-from-sun',
      look: 'around',
      title: { ca: 'La llum, al minut previ', es: 'La luz, en el minuto previo', en: 'The light, one minute before maximum', fr: 'La lumière, une minute avant le maximum' },
      ...
      text: deep
        ? {
            ...
            en: `With ${pct}% of the Sun covered and the Sun at ${Math.round(sunAlt)}°, this app’s light model gives about ${lux} lux: like ${comparison.en}, around ${factor} times less light than without an eclipse. It really will become dark, and that is the trap: it will resemble totality enough to tempt someone to remove the filter. It is not totality. A sliver of photosphere remains lit until the final second, enough to burn the retina and wash out the corona —the sight you are truly missing.`, fr: `Avec ${pct} % du Soleil couvert et le Soleil à ${Math.round(sunAlt)}°, le modèle lumineux de cette application donne environ ${lux} lux : comme ${comparison.fr}, soit environ ${factor} fois moins de lumière que sans éclipse. Il fera réellement sombre, et c’est là le piège : cela ressemblera assez à une totalité pour inciter quelqu’un à retirer son filtre. Ce n’est pas une totalité. Un fin croissant de photosphère reste lumineux jusqu’à la dernière seconde, assez pour brûler la rétine et effacer la couronne — précisément ce que vous manquez.`,
          }
        : {
            ...
            en: `With ${pct}% of the Sun covered, the model gives about ${lux} lux, around ${factor} times less light than without an eclipse. You will notice much less change than the figure suggests: the eye responds logarithmically and your pupils have been dilating throughout the partial phase. What does stand out is the quality of the light and the sharpness of the shadows.`, fr: `Avec ${pct}% du Soleil couvert, le modèle donne environ ${lux} lux, soit environ ${factor} fois moins de lumière que sans éclipse. Vous remarquerez beaucoup moins de changement que ce que suggère la figure : l’œil réagit de manière logarithmique et vos pupilles se sont dilatées tout au long de la phase partielle. Ce qui ressort, c'est la qualité de la lumière et la netteté des ombres.`,
          },
      speech: {
        ca: 'Mira la qualitat de la llum i les ombres, no el Sol.',
        es: 'Mira la calidad de la luz y las sombras, no el Sol.',
        en: 'Look at the quality of the light and the shadows, not the Sun.', fr: 'Regardez la qualité de la lumière et des ombres, pas le Soleil.',
      },
    },
    {
      id: 'max-filtered',
      ...
      look: 'sun',
      essential: true,
      title: { ca: `Màxim · ${pct} %`, es: `Máximo · ${pct} %`, en: `Maximum · ${pct}%`, fr: `Maximum · ${pct}%` },
      text: {
        ...
        en: `Eclipse maximum from this location: ${pct}% of the solar disc covered. Through the filter, the Sun is at its narrowest of the entire event.`, fr: `Eclipse maximale à partir de cet endroit : ${pct}% du disque solaire couvert. À travers le filtre, le Soleil est à son point le plus étroit de tout l’événement.`,
      },
      speech: { ca: 'Màxim de l’eclipsi. No et treguis el filtre.', es: 'Máximo del eclipse. No te quites el filtro.', en: 'Eclipse maximum. Keep the filter on.', fr: 'Éclipse maximale. Gardez le filtre en place.' },
    },
    {
      id: 'temperature',
      anchor: 'max',
      ...
      text: {
        ...
        en: 'The temperature falls and birds grow quiet. With high obscuration the effect is clear even if the light seems normal. Listen as much as you look.', fr: 'La température baisse et les oiseaux se taisent. Avec une obscuration élevée, l\'effet est clair même si la lumière semble normale. Écoutez autant que vous regardez.',
      },
      speech: { ca: 'Escolta. Baixa la temperatura.', es: 'Escucha. Baja la temperatura.', en: 'Listen. The temperature is falling.', fr: 'Écoutez. La température baisse.' },
    },
  ];
  ...
  if (deep && shadow) {
    ...
    drafts.push({
      ...
      filterState: 'away-from-sun',
      look: 'horizon',
      title: { ca: 'L’ombra, de lluny', es: 'La sombra, de lejos', en: 'The shadow in the distance', fr: 'L\'ombre au loin' },
      text: {
        ...
        en: `The Moon’s shadow passes nearby, but not here. Look towards the ${cardinalEn}: the horizon darkens like a storm that never arrives. This is the closest you will come to totality from this location.`, fr: `L’ombre de la Lune passe à proximité, mais pas ici. Regardez vers ${cardinalFr} : l’horizon s’assombrit comme un orage qui n’arrive jamais. C’est ce qui se rapproche le plus de la totalité depuis cet emplacement.`,
      },
      speech: { ca: `Mira cap ${towardsCa(cardinalCa)}. L’ombra passa de llarg.`, es: `Mira hacia el ${cardinalEs}. La sombra pasa de largo.`, en: `Look towards the ${cardinalEn}. The shadow passes by.`, fr: `Regardez vers ${cardinalFr}. L’ombre passe au loin.` },
    });
  }
  ...
  if (circumstances.edgeUncertain) {
    ...
    out.push({
      ...
      text: {
        ...
        en: `You are at the edge of the path: your margin from the limit is ${arcsec}″, while the ephemeris error in the Sun–Moon relative position is about 1.5″. We cannot honestly say whether a central phase will occur here. Move further into the path.`, fr: `Vous êtes au bord du chemin : votre marge par rapport à la limite est de ${arcsec}″, tandis que l'erreur des éphémérides dans la position relative Soleil-Lune est d'environ 1,5″. Nous ne pouvons honnêtement pas dire si une phase centrale aura lieu ici. Avancez plus loin dans le chemin.`,
      },
    });
  }
  ...
  if (variant === 'totality' || variant === 'annular') {
    out.push({
      ...
      text: {
        ...
        en: `Contact times are calculated using the mean radii of the Sun and Moon: the Moon’s real limb relief reaches about ±2 km and shifts them by a couple of seconds. Compared with IGN tables, our second contact is also several seconds early —up to nine in the worst measured case— and the error consistently points the same way. That is why the instruction to remove the filter does not arrive until ${FILTER_OFF_DELAY_SEC} seconds after calculated C2, and is still conditional on darkness having fallen and the corona being visible: if any point of light remains, wait. Replace the filter fifteen seconds before C3.`, fr: `Les temps de contact sont calculés à partir des rayons moyens du Soleil et de la Lune : le relief réel des membres de la Lune atteint environ ±2 km et les décale de quelques secondes. Par rapport aux tables IGN, notre deuxième contact est également en avance de plusieurs secondes – jusqu'à neuf dans le pire des cas mesurés – et l'erreur pointe systématiquement dans la même direction. C'est pourquoi l'instruction de retirer le filtre n'arrive que ${FILTER_OFF_DELAY_SEC} secondes après le calcul de C2, et reste toujours conditionnée à la tombée de l'obscurité et à la visibilité de la couronne : s'il reste un point lumineux, attendez. Remplacez le filtre quinze secondes avant C3.`,
      },
    });
  }
  ...
  if (variant === 'totality') {
    out.push({
      ...
      text: {
        ...
        en: 'Shadow bands are caused by atmospheric turbulence, not geometry: they cannot be predicted and do not appear at every eclipse.', fr: 'Les bandes d\'ombre sont causées par les turbulences atmosphériques et non par la géométrie : elles ne peuvent pas être prédites et n\'apparaissent pas à chaque éclipse.',
      },
    });
  }
  ...
  if (shadow?.lowSunCaveat) {
    out.push({
      ...
      text: {
        ...
        en: `The Sun is ${Math.round(shadow.sunAltitudeDeg)}° above the horizon. The shadow arrives at such a shallow angle that the wall of darkness is diffuse and blends into dusk. It will be visible, but not like photographs of eclipses with the Sun high in the sky.`, fr: `Le Soleil est à ${Math.round(shadow.sunAltitudeDeg)}° au-dessus de l'horizon. L’ombre arrive sous un angle si faible que le mur d’obscurité est diffus et se fond dans le crépuscule. Ce sera visible, mais pas comme les photographies d’éclipses avec le Soleil haut dans le ciel.`,
      },
    });
  }
  ...
  if (shadow?.speedDiverging) {
    out.push({
      ...
      text: {
        ...
        en: 'With the Sun this low, the shadow’s ground speed soars and ceases to be a useful figure. That is why no number is shown here.', fr: 'Avec un Soleil aussi bas, la vitesse au sol de l’ombre monte en flèche et cesse d’être un chiffre utile. C\'est pourquoi aucun numéro n\'est affiché ici.',
      },
    });
  }
